...
2. 「性格の中核」：重要度の高い特徴を3つ。各特徴について、根拠となる因子・下位尺度と、日常で表れそうな具体例を示す
...
- 美容誌、ライフスタイル誌、ブランドの診断コンテンツのような、清潔感と編集感のあるレイアウト
- 特定の作品を再現せず、情報が整理された診断レポートというデザインジャンルのみを参考にする
- 大小の情報ブロックを組み合わせ、単調な5段カード構成にはしない
- 余白を十分に使いながら、1枚の中に情報量とメリハリを持たせる

【配色・デザイン】
- オフホワイトを基調に、濃い文字色と落ち着いたアクセントカラー1〜2色で構成する
- スマートフォンで読める大きさの日本語を使い、見出し・本文・数値の情報階層を明確にする
- 日本語を中心とし、意味のない英語装飾や過剰なイラストは入れない
- 診断傾向からAIが考えた架空のペルソナを、顔写真風のエディトリアルポートレートとして配置してよい
- ペルソナは性格の雰囲気を象徴する存在として扱い、診断結果から実際の容姿、年齢、性別などを断定しない
...
- 下位尺度の数値は画像内に羅列せず、文章を考えるための根拠として使用する
...
- 高低だけを順番に説明せず、目立つ因子、平均付近の因子、下位尺度のばらつきを比較してください。
- 一見矛盾するスコアがあれば、矛盾として処理せず「状況によって切り替わる特徴」として解釈してください。
- 各解釈には、根拠となる因子または下位尺度を自然な形で示してください。
