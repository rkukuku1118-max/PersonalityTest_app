...
質問文は、公式日本語版と英語原文の意味対応を保ちながら、重複表現、文字化け、現在の日本語として不自然な構文を必要最小限だけ調整しています。頻度や程度を表す語、否定、条件、逆転項目の向きは変更しません。文言を変更した版は公式日本語版そのものではなく、心理測定上の同等性を保証するには別途検証が必要です。
...
このチェックでは、質問数、採点対象の質問番号、基準値の参照、重複や欠落に加え、60問版と100問版で共有する項目の文言が一致していることを確認します。
